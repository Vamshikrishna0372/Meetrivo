...
import { u as useWebSocket } from "./useWebSocket-G0Qj-KKH.mjs";
import { meetings, recordings, files, breakoutRooms, ai } from "./apiClient-DhOx7IPH.mjs";
...
function RoomPage() {
  ...
  const [urlMeetingId, setUrlMeetingId] = reactExports.useState("");
  ...
  reactExports.useEffect(() => {
    if (urlMeetingId) {
      ...
      meetings.getById(urlMeetingId).then((res) => {
        if (res) {
  ...
  const fetchByCode = (code) => {
    ...
    meetings.searchByCode(code).then((res) => {
      if (res && res.length > 0) {
      ...
      console.error(`[DEBUG][ROOM] searchByCode error:`, err);
    });
  };
  reactExports.useEffect(() => {
  ...
  const getOrCreatePeerConnection = reactExports.useCallback((userId) => {
  ...
  const handleWebRTCSignal = reactExports.useCallback(async (msg) => {
    const {
      type,
      senderId,
      payload
  ...
  const initiateCall = reactExports.useCallback((userId) => {
