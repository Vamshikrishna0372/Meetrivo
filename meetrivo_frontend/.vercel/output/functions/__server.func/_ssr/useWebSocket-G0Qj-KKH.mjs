...
import { meetings } from "./apiClient-DhOx7IPH.mjs";
const MAX_RETRIES = 5;
...
function useWebSocket(meetingId, onMessageReceived, onSignalReceived, onWhiteboardReceived) {
  ...
  const onMessageReceivedRef = reactExports.useRef(onMessageReceived);
  const onSignalReceivedRef = reactExports.useRef(onSignalReceived);
  const onWhiteboardReceivedRef = reactExports.useRef(onWhiteboardReceived);
...
`;
    ...
    socket.onmessage = (event) => {
      ...
      if (frame.command === "CONNECTED") {
        ...
        meetings.getParticipants(meetingId).then((data) => {
          console.log(`[DEBUG][WS] REST participants for meetingId=${meetingId}:`, data?.map((p) => p.userId || p.id));
        ...
        try {
          ...
          if (dest.includes("/topic/chat/")) {
            ...
            console.log(`[DEBUG][WS] PARTICIPANTS event=${eventType} currentParts=`, currentParts?.map((p) => p.userId || p.id));
  ...
  const sendRaw = reactExports.useCallback((destination, payload) => {
...
` + JSON.stringify(payload) + "\0"
  ...
  const sendMessage = reactExports.useCallback(
    (content) => sendRaw(`/app/chat/${meetingId}/message`, { message: content, receiverId: null }),
    [meetingId, sendRaw]
  );
  ...
  const sendHand = reactExports.useCallback(
    (raised) => sendRaw(`/app/meeting/${meetingId}/media/hand`, { enabled: raised }),
    [meetingId, sendRaw]
  );
  const sendSignal = reactExports.useCallback(
    (signalType, receiverId, payload) => sendRaw(`/app/meeting/${meetingId}/signaling`, {
      type: signalType,
      meetingId,
      receiverId,
      payload
  ...
  const updateState = reactExports.useCallback((micOn, camOn, screenShare) => {
  ...
  const sendWhiteboardEvent = reactExports.useCallback(
    (eventType, data) => sendRaw(`/app/whiteboard/${meetingId}/event`, { eventType, ...data }),
    [meetingId, sendRaw]
  );
  return {
    connected,
