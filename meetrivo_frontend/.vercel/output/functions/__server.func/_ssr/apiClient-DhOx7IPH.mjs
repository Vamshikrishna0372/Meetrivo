...
const admin = {
  ...
  getUserAnalytics: (userId) => apiFetch(`/api/admin/users/${userId}/analytics`),
  getUserLoginHistory: (userId) => apiFetch(`/api/admin/users/${userId}/login-history`),
  getUserActivity: (userId, limit = 100) => apiFetch(`/api/admin/users/${userId}/activity?limit=${limit}`),
  // Bulk actions
  bulkAction: (userIds, action, role, targetId) => apiFetch("/api/admin/users/bulk", {
    method: "POST",
