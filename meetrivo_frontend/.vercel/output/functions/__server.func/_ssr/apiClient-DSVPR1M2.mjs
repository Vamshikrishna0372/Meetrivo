...
const auth = {
  ...
  // OAuth2
  getOAuthUrl: (provider) => `${BACKEND_URL}/api/auth/oauth2/authorize/${provider}`,
  oauth2Login: (data) => apiFetch("/api/auth/oauth2/login", {
    method: "POST",
...
const users = {
  ...
  getUserStats: () => apiFetch("/api/users/stats")
};
const meetings = {
  // Get all meetings hosted by current user
  getAll: () => apiFetch("/api/meetings/my"),
  getById: (id) => apiFetch(`/api/meetings/${id}`),
  getParticipants: (id) => apiFetch(`/api/meetings/${id}/participants`),
  getWaitingRoom: (id) => apiFetch(`/api/meetings/${id}/waiting-room`),
  ...
  start: (id) => apiFetch(`/api/meetings/${id}/start`, { method: "POST" }),
  // Upcoming meetings from dashboard endpoint
  getUpcoming: () => apiFetch("/api/dashboard/upcoming-meetings"),
  // Recent meetings from dashboard endpoint
  getRecent: () => apiFetch("/api/dashboard/recent-meetings"),
  ...
  approveParticipant: (meetingId, userId) => apiFetch(`/api/meetings/${meetingId}/approve/${userId}`, { method: "POST" }),
  // Reject participant from waiting room
  rejectParticipant: (meetingId, userId) => apiFetch(`/api/meetings/${meetingId}/reject/${userId}`, { method: "POST" }),
  // Remove participant
  removeParticipant: (meetingId, userId) => apiFetch(`/api/meetings/${meetingId}/remove/${userId}`, { method: "POST" }),
  // Ban participant
  banParticipant: (meetingId, userId) => apiFetch(`/api/meetings/${meetingId}/ban/${userId}`, { method: "POST" }),
  // Assign role (CO_HOST, MODERATOR)
  assignRole: (meetingId, userId, role) => apiFetch(`/api/meetings/${meetingId}/assign-role/${userId}?role=${role}`, { method: "POST" }),
  // Change meeting password
  changePassword: (meetingId, password) => apiFetch(`/api/meetings/${meetingId}/password`, { method: "POST", body: JSON.stringify({ password }) })
};
const notifications = {
...
const analytics = {
  ...
  getDashboard: () => apiFetch("/api/users/stats"),
  getEngagement: () => apiFetch("/api/admin/analytics/meetings"),
  getMeetingStats: (meetingId) => apiFetch(`/api/admin/analytics/meetings`),
  getUsage: () => apiFetch("/api/admin/analytics/system"),
  // Per-organization analytics
  getOrgAnalytics: (orgId) => apiFetch(`/api/organizations/${orgId}/analytics`),
  // Admin-only full analytics
  getAdminDashboard: () => apiFetch("/api/admin/analytics/users")
};
const organizations = {
  ...
  delete: (id) => apiFetch(`/api/organizations/${id}`, { method: "DELETE" }),
...
const teams = {
  ...
  addMember: (teamId, userId) => apiFetch(`/api/teams/${teamId}/members/${userId}`, {
    method: "POST"
  }),
  removeMember: (teamId, userId) => apiFetch(`/api/teams/${teamId}/members/${userId}`, { method: "DELETE" }),
  transferManager: (teamId, newManagerId) => apiFetch(`/api/teams/${teamId}/manager/${newManagerId}`, { method: "PUT" }),
  getMembers: (teamId) => apiFetch(`/api/teams/${teamId}/members`)
};
const departments = {
  ...
  assignHead: (id, headId) => apiFetch(`/api/departments/${id}/head/${headId}`, { method: "PUT" }),
  assignMembers: (id, memberIds) => apiFetch(`/api/departments/${id}/members`, { method: "PUT", body: JSON.stringify(memberIds) })
};
const recordings = {
  // My recordings
  getAll: () => apiFetch("/api/recordings/my"),
  // Get recordings for a specific meeting
  getForMeeting: (meetingId) => apiFetch(`/api/recordings/meeting/${meetingId}`),
  getById: (id) => apiFetch(`/api/recordings/${id}`),
  // Backend uses POST /api/recordings/start with body
  start: (meetingId, recordingType, permission) => apiFetch(`/api/recordings/start`, {
    method: "POST",
    body: JSON.stringify({
      meetingId,
  ...
  stop: (recordingId) => apiFetch(`/api/recordings/stop?recordingId=${encodeURIComponent(recordingId)}`, { method: "POST" }),
  pause: (recordingId) => apiFetch(`/api/recordings/pause?recordingId=${encodeURIComponent(recordingId)}`, { method: "POST" }),
  resume: (recordingId) => apiFetch(`/api/recordings/resume?recordingId=${encodeURIComponent(recordingId)}`, { method: "POST" }),
  delete: (id) => apiFetch(`/api/recordings/${id}`, { method: "DELETE" }),
  // Playback streaming URL
  getPlayUrl: (id) => `${BACKEND_URL}/api/recordings/play/${id}`,
  // Download URL with token
  download: (id) => `${BACKEND_URL}/api/recordings/download/${id}`
};
const files = {
...
const billing = {
  ...
  getPlan: (planId) => apiFetch(`/api/billing/plans/${planId}`),
  ...
  upgrade: (planId) => apiFetch("/api/billing/upgrade", { method: "PUT", body: JSON.stringify({ planId }) }),
  downgrade: (planId) => apiFetch("/api/billing/downgrade", { method: "PUT", body: JSON.stringify({ planId }) }),
  renew: () => apiFetch("/api/billing/renew", { method: "POST" }),
  getInvoices: () => apiFetch("/api/invoices"),
  getInvoice: (id) => apiFetch(`/api/invoices/${id}`),
  downloadInvoiceUrl: (id) => `${BACKEND_URL}/api/invoices/download/${id}`,
...
const admin = {
  ...
  getUserById: (userId) => apiFetch(`/api/admin/users/${userId}`),
  ...
  // System health
  getSystemHealth: () => apiFetch("/api/system/health"),
  // Support tickets (admin view)
  getAllSupportTickets: () => apiFetch("/api/support"),
  updateSupportTicketStatus: (ticketId, status) => apiFetch(`/api/support/${ticketId}/status`, { method: "PUT", body: JSON.stringify({ status }) }),
  // Feedback (admin view)
  getAllFeedback: () => apiFetch("/api/feedback"),
  // Announcements
  getAnnouncements: () => apiFetch("/api/admin/announcements"),
  createAnnouncement: (data) => apiFetch("/api/admin/announcements", { method: "POST", body: JSON.stringify(data) })
};
